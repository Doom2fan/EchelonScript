...
public unsafe sealed class RoslynBackendData : IBackendData {
    ...
    public T? GetFunctionDelegate<T> ([DisallowNull] ES_FunctionData* func) where T : Delegate {
        ...
        if (Delegate.CreateDelegate (typeof (T), methodInfo, false) is not T ret)
            return null;
...
public unsafe sealed partial class RoslynCompilerBackend : ICompilerBackend, IDisposable {
    ...
    internal ref struct PassData {
        ...
        public Span<StructPooledList<MemberDeclarationSyntax>> GlobalMembersInit { private get; init; }
        public ref StructPooledList<MemberDeclarationSyntax> GlobalMembers => ref GlobalMembersInit [0];
...
#pragma warning disable NoCopy11 // non-copyable
                MappingsInit = MemoryMarshal.CreateSpan (ref mappings, 1),
                GlobalMembersInit = MemoryMarshal.CreateSpan (ref members, 1),
                TypesInit = MemoryMarshal.CreateSpan (ref types, 1),
#pragma warning restore NoCopy11 // non-copyable
    ...
    private static bool CompileCSharp (ref PassData passData, ArrayPointer<nint> gcRoots) {
        ...
        var compUnit = CompilationUnit ().WithMembers (
            ...
            UsingDirective (NamespaceNameSyntax ("EchelonScript.Common")),
            UsingDirective (NamespaceNameSyntax ("EchelonScript.Common", "Data")),
            UsingDirective (NamespaceNameSyntax ("EchelonScript.Common", "Data", "Types")),
            UsingDirective (NamespaceNameSyntax ("EchelonScript.Common", "GarbageCollection")),
            UsingDirective (NamespaceNameSyntax ("EchelonScript.Common", "Utilities")),
    ...
    private static ExpressionSyntax PointerLiteral (void* value, TypeSyntax? type = null) {
        ...
        var litToken = sizeof (void*) switch {
            sizeof (uint) => Literal ((uint) value),
            sizeof (ulong) => Literal ((ulong) value),

            _ => throw new NotImplementedException ("Pointer size not implemented."),
        };
