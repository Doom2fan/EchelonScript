...
internal abstract class ESIR_NodeCacheBase {
    ...
    internal ESIR_NodeCacheBase () => buckets = new CacheBucket [CacheSize];
...
internal class ESIR_NodeCache : ESIR_NodeCacheBase {
    ...
    private static bool IsEquivalent (ESIR_Node node, ESIR_NodeKind kind)
        => node.Kind == kind && node.ChildrenCount == 0;
